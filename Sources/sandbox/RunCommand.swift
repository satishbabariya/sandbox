...
struct RunCommand: AsyncParsableCommand {
    ...
    @Argument(
        help: "Agent name (see 'sandbox agents ls') or a container image. Defaults to the configured agent."
    )
    var target: String?
    ...
    /// Phase timings, printed when SANDBOX_TRACE is set.
    ...
    func run() async throws {
        ...
        for (url, error) in registry.profiles().broken {
            FileHandle.standardError.write(
                Data(
                    "sandbox: ignoring agent profile \(url.lastPathComponent): \(error)\n"
                        .utf8))
        }
        let gateway = InstallLayout.gatewayBinary()
        let config = try SandboxConfig.load(paths)

        // A bare `sandbox run` uses the configured default agent, so the
        ...
        guard let target = self.target ?? config.defaultAgent else {
            throw ValidationError(
                "no agent or image given, and no defaultAgent configured; "
                    + "try 'sandbox run shell' or 'sandbox config set defaultAgent claude'")
        }

        // `sandbox run claude` and `sandbox run alpine:3.20` are both valid.
        ...
        let id =
            ...
            ?? "sandbox-\(UInt32.random(in: 0..<0xFFFF_FFFF))"
        ...
        if !mcp.isEmpty, profile == nil {
            throw ValidationError(
                "--mcp needs an agent: MCP servers are installed into an agent's "
                    ...
                    + "Run an agent (sandbox agents ls), or add the server to a "
                    + "profile with: sandbox agents edit <name>")
        }
        ...
        if let existing = try? store.load(id), existing.state == .running {
            throw CleanExit.message(
                "sandbox '\(id)' is already running; use sandbox exec \(id) -- <cmd>")
        }
        ...
        for service in effectiveSecrets {
            ...
            // describe a service sandbox has no preset for and can name more
            // than one domain.
            ...
            guard !resolved.isEmpty else {
                throw ValidationError(
                    "no binding for secret '\(service)'; sandbox has no preset for it "
        ...
        if policy.allow.isEmpty && effectiveSecrets.isEmpty {
            FileHandle.standardError.write(
                Data("sandbox: no --allow rules; this sandbox reaches nothing\n".utf8))
        }
        ...
        if let instructions = profile?.agentInstructions, !effectiveClone {
            FileHandle.standardError.write(
                Data(
                    ("sandbox: not writing \(instructions.filename); it would add a file "
        ...
        if let profile = profileWithMCP, !profile.allInstall.isEmpty, template == nil {
            ...
            if rebuild || !cache.isCached(profile) {
                FileHandle.standardError.write(
                    Data("sandbox: preparing '\(profile.name)' (first run only)\n".utf8))
            }
        ...
        // litter `sandbox ls` with entries nobody can act on — including when
        ...
        do {
            ...
            // an image nobody meant to pull -- an HTTP 401 for `sandbox run
        ...
        for forward in forwards {
            FileHandle.standardError.write(
                Data("sandbox: published \(forward)\n".utf8))
        }
    ...
    static func spawnSupervisor(
        launch: LaunchSpec, paths: SandboxPaths, store: SandboxStore
    ) async throws {
...
enum InstallLayout {
    static func gatewayBinary() -> URL {
        ...
        for _ in 0..<5 {
            for candidate in [
                directory.appending(path: "gvsandbox"),
                directory.appending(path: "bin/gvsandbox"),
        ...
        return exe.deletingLastPathComponent().appending(path: "gvsandbox")
    }
}
