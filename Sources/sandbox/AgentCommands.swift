...
struct AgentsCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: """
            ...
            credential it needs, so 'sandbox run claude' works without knowing \
            any of them. Drop JSON in ~/.sandbox/agents to add your own or \
...
struct AgentsEdit: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "edit",
        abstract: "Write an agent's definition to ~/.sandbox/agents so you can change it."
    )
    ...
    func run() async throws {
        ...
        print("edit it, then: sandbox run \(name)")
    }
}
...
/// otherwise trying a kit means editing ~/.sandbox/agents by hand afterwards.
