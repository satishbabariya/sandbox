...
/// User defaults, read from `~/.airlock/config.json`.
///
...
public struct AirlockConfig: Codable, Sendable, Equatable {
    /// Agent used when `airlock run` is given no target.
    ...
    public static func path(_ paths: AirlockPaths = AirlockPaths()) -> URL {
    ...
    public static func load(_ paths: AirlockPaths = AirlockPaths()) throws -> AirlockConfig {
        let url = path(paths)
        guard let data = try? Data(contentsOf: url) else { return AirlockConfig() }
        do {
            return try JSONDecoder().decode(AirlockConfig.self, from: data)
        } catch {
    ...
    public var description: String {
        ...
        case .malformed(let url, let underlying):
            return """
                ...
                fix or delete it — airlock will not ignore a config it cannot read,
