...
public enum NetstackError: Error, CustomStringConvertible {
    ...
    public var description: String {
        ...
        case .binaryNotFound(let url):
            return "gvsandbox not found at \(url.path); build it with: make -C netstack"
        case .didNotStart(let detail):
            return "gvsandbox did not start: \(detail)"
        case .exitedEarly(let code, let detail):
            guard let detail, !detail.isEmpty else {
                return "gvsandbox exited with status \(code) before it was ready"
            }
            return "gvsandbox exited before it was ready: \(detail)"
        }
    }
}

/// Owns the `gvsandbox` gateway process for exactly one sandbox.
///
...
public actor NetstackSupervisor {
    public struct Configuration: Sendable {
        ...
        public init(
            ...
            subnet: String = SandboxInterface.Defaults.subnet,
            gatewayIP: String = SandboxInterface.Defaults.gateway,
    ...
    public nonisolated var caCertificatePath: URL {
        guestShareDirectory.appending(path: "sandbox-ca.crt")
    }
    ...
    public static func defaultBinary(installRoot: URL) -> URL {
        installRoot.appending(path: "bin/gvsandbox")
    }
    ...
    public func start() async throws -> GuestLink {
        ...
        // Recorded so the gateway can be reaped if sandbox never gets to stop
        // it. A gateway is a child process: kill sandbox with a signal it
    ...
    private var forwardLines: String {
        ...
        let entries = config.ports.map { forward in
            "    \"\(forward.hostAddress):\(forward.hostPort)\": "
                + "\"\(SandboxInterface.Defaults.guest):\(forward.guestPort)\""
        }.joined(separator: "\n")
        return "  forwards:\n" + entries
    }
