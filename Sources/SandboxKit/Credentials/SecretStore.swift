...
/// the read blocks forever: `sandbox run claude` sat for nineteen minutes with
    ...
    public var description: String {
        ...
        case .needsAuthorisation(let service):
            return """
                ...
                run `sandbox secret check \(service)` once in a terminal and choose \
                Always Allow
                """
        case .invalidValue:
...
public struct SecretStore: Sendable {
    /// Namespaced so sandbox's entries are distinguishable from anything else
    /// in the user's keychain.
    public static let servicePrefix = "com.sandbox.secret."
