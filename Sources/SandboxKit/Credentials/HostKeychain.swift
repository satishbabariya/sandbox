...
public enum CredentialSource: Codable, Sendable, Equatable {
    /// A value the user stored with `sandbox secret set`.
    case sandboxKeychain
...
extension CredentialBinding {
    /// Bindings that reuse a sign-in the user has already done, so `sandbox run
    /// claude` works without a separate `sandbox secret set`.
