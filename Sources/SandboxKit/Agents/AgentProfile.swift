...
///
/// The point is that `sandbox run claude` should work without the user knowing
...
public struct AgentProfile: Codable, Sendable, Equatable {
    ...
    /// Bindings for services sandbox has no preset for, or that need more than
    /// one domain.
    ///
    /// A kit states the environment variable, domain, header and format for
    /// every credential it uses, so it can describe a service sandbox has
    ...
    /// an older sandbox must keep loading after a field is added -- the
    ...
    public var description: String {
        ...
        case .notCustom(let name):
            return """
                ...
                built-ins are part of sandbox; customise one with: sandbox agents edit \(name)
                """
...
public struct AgentRegistry: Sendable {
    ...
    /// only symptom was `sandbox run <name>` deciding the name must be an image
