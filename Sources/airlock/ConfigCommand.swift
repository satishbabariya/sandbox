...
struct ConfigCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: """
            Settings live in ~/.airlock/config.json. Flags override them, with \
...
struct ConfigSet: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: """
            List settings take repeated values:
              airlock config set allow '*.internal.corp' registry.example.com
            """
    )
    ...
    func run() async throws {
        var config = try AirlockConfig.load()
        guard let first = values.first else {
            throw ValidationError("no value given; use 'airlock config unset \(key)' to clear it")
        }
