...
struct DashboardCommand: AsyncParsableCommand {
    ...
    func run() async throws {
        guard isatty(STDOUT_FILENO) == 1 else {
            throw CleanExit.message(
                "airlock top needs a terminal; use 'airlock ls' when piping")
        }
    ...
    private func render(width: Int, height: Int) {
        ...
        lines.append(
            bold(
                "airlock  \(running.count) running, \(records.count) total"
                    .padding(toLength: max(0, width - 10), withPad: " ", startingAt: 0)
                    + "q quit"))
        lines.append("")

        if records.isEmpty {
            lines.append(dim("  no sandboxes — start one with: airlock run shell"))
        } else {
    ...
    private func latestDecisions(
        paths: AirlockPaths, sandboxes: [String]
