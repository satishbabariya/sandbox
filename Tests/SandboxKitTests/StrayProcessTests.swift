...
struct StrayProcessTests {
    ...
    func ours() {
        let supervisor = StrayProcess.parse(
            line: " 50708 /usr/local/bin/sandbox supervise --spec /tmp/sandbox-demo/launch.json")
        ...
        let gateway = StrayProcess.parse(
            line: "50709 /usr/local/bin/gvsandbox -config /tmp/sandbox-demo/gateway.yaml")
    ...
    func notOurs() {
        for line in [
            ...
            "58001 /bin/zsh -c sandbox supervise /tmp/sandbox-demo/launch.json",
            "58002 /bin/sh -c pgrep -f 'sandbox supervise' /tmp/sandbox-demo",
            "58003 grep gvsandbox /tmp/sandbox-demo/notes",
            "58004 /usr/bin/vim /tmp/sandbox-demo/gateway.yaml",
            "58005 tail -f /tmp/sandbox-demo/gateway.log",
            // Our binary, but not the supervisor -- someone's interactive run.
            "58006 /usr/local/bin/sandbox run alpine --name /tmp/sandbox-demo",
            "58007 /usr/local/bin/sandbox ls",
        ] {
    ...
    func noDirectory() {
        #expect(StrayProcess.parse(line: "1 /usr/local/bin/sandbox supervise --help") == nil)
        #expect(StrayProcess.parse(line: "2 /usr/local/bin/gvsandbox -version") == nil)
    }

    @Test("malformed lines are ignored rather than guessed at")
    func malformed() {
        for line in ["", "   ", "notapid /usr/local/bin/gvsandbox /tmp/sandbox-x", "12345"] {
    ...
    func liveSandboxSurvives() {
        let listing = """
             900 /usr/local/bin/sandbox supervise --spec /tmp/sandbox-live/launch.json
             901 /usr/local/bin/gvsandbox -config /tmp/sandbox-dead/gateway.yaml
            """
