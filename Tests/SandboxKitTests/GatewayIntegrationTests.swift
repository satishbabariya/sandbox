...
/// End-to-end against the real `gvsandbox` gateway, with no VM involved.
///
...
private func gvsandboxBinary() -> URL? {
    let root = URL(filePath: #filePath)
        .deletingLastPathComponent()
        .deletingLastPathComponent()
        .deletingLastPathComponent()
    let bin = root.appending(path: ".build/bin/gvsandbox")
...
@Suite("gvsandbox gateway", .serialized)
struct GatewayIntegrationTests {
    ...
    private func startGateway(
        ...
        guard let binary = gvsandboxBinary() else { return nil }
    ...
    func dhcpRoundTrip() throws {
        ...
        guard
            ...
            Issue.record("gvsandbox not built; run: make -C netstack")
            return
        }
        defer {
            proc.terminate()
